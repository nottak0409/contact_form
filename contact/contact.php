    ...
    <form id="form" action="confirm.php" method="post" name="form" onsubmit="return validate()">
      <div class="form-group">
        <label for="name">お名前（30文字以内)(必須)
          <span class="error"><?php if (isset ($_SESSION["error_name"] ) ) echo h($_SESSION["error_name"]); ?></span>
        </label>
        <input type="text" class="form-control" id="name" name="name" placeholder="氏名" value="<?php echo h($name) ?>">
      </div>
      <div class="form-group">
        <label for="email">メールアドレス(必須)
          <span class="error"><?php if (isset ($_SESSION["error_email"] ) ) echo h($_SESSION["error_email"]); ?></span>
        </label>
      ...
      <div class="form-group">
        <label for="email_check">メールアドレス(確認)
          <span class="error"><?php if (isset ($_SESSION["error_email_check"] ) ) echo h($_SESSION["error_email_check"]); ?></span>
        </label>
      ...
      <div class="form-group">
        <label for="tel">お電話番号(半角英数字)(必須)
          <span class="error"><?php if (isset ($_SESSION["error_tel"] ) ) echo h($_SESSION["error_tel"]); ?></span>
        </label>
        <input type="text" class="form-control" id="tel" name="tel" placeholder="電話番号" value="<?php echo h($tel) ?>">
      </div>
      <div class="form-group">
        <label for="subject">件名(必須)
          <span class="error"><?php if (isset ($_SESSION["error_subject"])) echo h($_SESSION["error_subject"]); ?></span>
        </label>
      ...
      <div class="form-group">
        <label for="contact">お問い合わせ内容(必須)
          <span class="error"><?php if (isset ($_SESSION["error_contact"] ) ) echo h($_SESSION["error_contact"]); ?></span>
        </label>
